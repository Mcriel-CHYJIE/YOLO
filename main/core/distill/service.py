"""蒸馏业务逻辑 — 配置构建与验证 + 蒸馏工作线程"""
from pathlib import Path
from main.core.base import ROOT, DATA_YAML, cfg
...
def build_distill_config(
    teacher_path: str, student_path: str, data_path: str,
    epochs: int, batch: int, lr0: float, alpha: float,
    patience: int, imgsz: int, gpu_ok: bool,
    warmup_epochs: int, weight_decay: float, momentum: float,
) -> dict:
    """构建蒸馏配置参数"""
    return dict(
        ...
        workers=__import__("multiprocessing").cpu_count(),
        warmup_epochs=warmup_epochs,
        weight_decay=weight_decay, momentum=momentum,
    )


def load_distill_defaults() -> dict:
    """从 project.yaml 读取蒸馏默认值"""
    d = cfg.get('distill', {})
    if not d:
        d = {}
    ...
    data_path = Path(DATA_YAML).as_posix()
    return dict(
        teacher=teacher_path,
        student=student_path,
        data=data_path,
        epochs=d.get('epochs', 150),
        batch=d.get('batch', 24),
        lr0=d.get('lr0', 0.002),
        alpha=d.get('alpha', 0.5),
        patience=d.get('patience', 40),
        imgsz=str(d.get('imgsz', 640)),
...
# ══════════════════════════════════════════════════════════════
# DistillationLossWrapper — 蒸馏损失包装器
# ══════════════════════════════════════════════════════════════
...
class Distiller(QObject):
    ...
    def _run(self):
        ...
        self.log.emit(f'[Data] {cfg["config"]}')
