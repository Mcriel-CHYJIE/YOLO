...
class ValidateTab(QWidget):
    ...
    def _build(self):
        ui_path = Path(__file__).resolve().parent.parent / 'ui' / 'validate.ui'
    ...
    def _run_val(self):
        ...
        self._val_btn.setText('Validating…')
        ...
        class V(QThread):
            ...
            def run(self):
                try:
                    ...
                    v = cfg['validation']
                    r = YOLO(str(w)).val(data=DATA_YAML, device='0' if self.parent_gpu else 'cpu',
        ...
        vw = V()
        vw.parent_gpu = self.studio.gpu_ok
        vw.d.connect(self._vd)
        self.validator = vw
        vw.start()

    def _vd(self, r):
        self._val_btn.setEnabled(True)
        self._val_btn.setText('Run Validation')
        self.validator = None
        ...
        try:
            b = r.box
            self._vm50.setText(f'{b.map50:.4f}')
            self._vm95.setText(f'{b.map:.4f}')
            self._vp.setText(f'{b.p.mean():.4f}')
            self._vr.setText(f'{b.r.mean():.4f}')
            rows = ''
